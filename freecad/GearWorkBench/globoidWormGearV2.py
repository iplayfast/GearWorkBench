"""
Globoid Worm Gear V2 — Direct Toroidal-Spiral Construction
==========================================================

V2 abandons the hobbing-by-boolean simulation (see globoidWormGear.py for the
original) in favour of building the worm thread DIRECTLY from its closed-form
parametric equations — the toroidal-spiral model used by the OTVINTA globoid
calculator (https://www.otvinta.com/globoid.html).
...
NOTE: V2 builds the WORM only.  The matching wheel is the next phase (the
...
class GloboidWormGearV2Result:
    def __init__(self, obj, varset):
        ...
        self.Type = "GloboidWormGearV2Result"
    ...
    def _build_worm_shape(self, p):
        ...
        App.Console.PrintMessage(
            "[WormV2] arc=%.2f deg  R=%.2f  tip/root@waist=%.2f/%.2f  "
        ...
        try:
            ...
            App.Console.PrintWarning(
                "[WormV2] makeLoft failed (%s); using ruled-surface fallback\n" % e)
    ...
    def _rebuild(self):
        ...
        try:
            ...
            if problems:
                ...
                App.Console.PrintError("[WormV2] " + "; ".join(problems) + "\n")
                return
            ...
            App.Console.PrintMessage("[WormV2] done (valid=%s)\n" % shape.isValid())

        except Exception:
            import traceback
            App.Console.PrintError("[WormV2] rebuild failed:\n" + traceback.format_exc())
            try:
    ...
    def _make_wheel_base(self, doc):
        ...
        App.Console.PrintMessage(
            "[WormV2] wheel helix=%.2f deg (=lead), sketch rot=%.3f deg\n"
...
class GloboidWormGearV2Command:
    def GetResources(self):
        return {"Pixmap": mainIcon,
                "MenuText": "Create Globoid Worm Gear V2",
                "ToolTip": "Create globoid worm (direct toroidal-spiral surface, V2)"}
